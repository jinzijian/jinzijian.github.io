...
网站默认英文。英文文章使用不带语言后缀的网址，例如 `my-post.md`，填写 `lang: "en"`。中文版使用 `my-post-zh.md`，填写 `lang: "zh-CN"` 和 `translationOf: "my-post"`。两篇都设为 `draft: false` 后，文章顶部会自动显示“English / 中文”切换；列表默认显示英文标题和摘要，并附上两个语言入口。两个版本的标题、摘要、正文及图注分别在各自的 Markdown 文件中维护。
